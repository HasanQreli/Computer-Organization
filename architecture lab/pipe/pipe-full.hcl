...
# I set the pc to W_valM if W_icode is IJTAB similar to the ret case since they both get the next instruction address from memory.
# In fetch stage I added the instruction to instr_valid since it is a valid instruction, need_regids since it has a register and need_valC since it also has a constant.
# In decode stage I only added it to srcB since it has %rB as its second argument and it does not edit any registers.
# In execute stage I added E_ValB and E_ValC because it is necessary to find the address by adding these two.
# In memory stage I added the instruction to mem_read since we need to read what is in our desired and use address M_valE as memory address which was what I computed in the execute stage. I did not edit the errors since they work as desired in the pdf.
# For Pipeline Register Control I again thought of jtab similar to ret since they both get next pc from memory at the same stage so while in decode, execute, and memory stage I stalled fetch and injected bubble to decode. Also for priority I implemented similar to ret.
